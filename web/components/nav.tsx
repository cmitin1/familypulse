...
export function BottomNav() {
  ...
  return (
    <nav className="sticky bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-40 mt-4 grid grid-cols-6 gap-1 rounded-xl border border-border bg-card/95 p-1.5 shadow-sm backdrop-blur">
      {items.map((item) => (
        <Link
          key={item.href}
          href={item.href}
